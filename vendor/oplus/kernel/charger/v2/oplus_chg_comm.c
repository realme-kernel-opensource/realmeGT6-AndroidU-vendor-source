...
static void oplus_hidden_soc_smooth(struct oplus_chg_comm *chip)
{
	...
	if (!chip->config.hidden_soc_switch || reserve_soc < RESERVE_SOC_MIN || reserve_soc > RESERVE_SOC_MAX ||
		hidden_soc_percent < HIDDEN_SOC_PERCENT_MIN || hidden_soc_percent > HIDDEN_SOC_PERCENT_MAX) {
	...
	chg_info("soc[%d %d %d %d %d %d %d] \n",soc,chip->soc,temp_soc,chip->config.reserve_soc,hidden_soc_smooth,hidden_soc_coefficient,hidden_soc_percent);
...
check_update_time:
	...
	if (chip->ui_soc != ui_soc) {
		...
		if (!chip->batt_full && ui_soc == 100 && charging &&
		    (chip->config.smooth_switch || chip->ffc_status == FFC_DEFAULT || chip->config.hidden_soc_switch) &&
...
static void oplus_comm_parse_hidden_soc_dt(struct oplus_chg_comm *chip)
{
	...
	if(config->hidden_soc_switch) {
		if(config->reserve_soc < RESERVE_SOC_MIN || config->reserve_soc > RESERVE_SOC_MAX ||
		   config->hidden_soc_percent > HIDDEN_SOC_PERCENT_MAX || config->hidden_soc_percent < HIDDEN_SOC_PERCENT_MIN) {
		   config->reserve_soc = RESERVE_SOC_OFF;
		   config->hidden_soc_switch = false;
		}
	}

	chg_info("hidden_soc_switch %d hidden_soc_percent %d \n", config->hidden_soc_switch, config->hidden_soc_percent);
}
