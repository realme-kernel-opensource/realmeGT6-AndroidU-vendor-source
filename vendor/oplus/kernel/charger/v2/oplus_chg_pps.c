...
#define PPS_STATUS_VLOT_SHAKE	500
	if (!chip->support_cp_ibus && chip->support_pps_status) {
...
#define OPLUS_PPS_LED_ON_CURRENT_DEBOUNCE (500)
	...
	if (!chip->support_cp_ibus) {
		...
		if (chip->support_pps_status) {
			rc = oplus_pps_get_pps_status_info(chip, &chip->pps_status_info);
			if (rc < 0) {
				chg_err("pps get src info error\n");
				chip->support_pps_status = false;
			} else {
				pps_curr_ma = PPS_STATUS_CUR(chip->pps_status_info) * 50;
				if (pps_curr_ma < OPLUS_PPS_STATUS_ABNORMAL) {
					curr_ma = (curr_ma < pps_curr_ma) ? pps_curr_ma : curr_ma;
					curr_mv = PPS_STATUS_VOLT(chip->pps_status_info) * 20;
					curr_ratio = 1;
					delta_curr_ok = OPLUS_PPS_OK_CURRENT_DEBOUNCE;
					chg_err("iin = %d, vchg = %d, vol_set_mv = %d\n", curr_ma, curr_mv, chip->vol_set_mv);
				} else {
					chip->support_pps_status = false;
				}
			}
		} else if (chip->pps_third_limit_en) {
			rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_LED_ON, &msg_data, true);
			if (!rc) {
				led_status = !!msg_data.intval;
				if (led_status && (chip->target_curr_ma >= chip->config.pps_target_curr_max_ma)) {
					vote(chip->pps_curr_votable, BASE_MAX_VOTER, true, (chip->config.pps_target_curr_max_ma - OPLUS_PPS_OVER_CURRENT_THD), false);
					chg_err("[%d,%d,%d]\n", led_status, chip->target_curr_ma, chip->config.pps_target_curr_max_ma);
				}

				if (!led_status && (get_client_vote(chip->pps_curr_votable, BASE_MAX_VOTER) < chip->config.pps_target_curr_max_ma)) {
					vote(chip->pps_curr_votable, BASE_MAX_VOTER, true, chip->config.pps_target_curr_max_ma, false);
					chg_err("[%d,%d,%d]\n", led_status, chip->config.pps_target_curr_max_ma, get_client_vote(chip->pps_curr_votable, BASE_MAX_VOTER));
				}
			}
		}
	} else {
...
#define IBAT_OVER_ITARGET_CNT (8)
...
static void oplus_pps_type_change_work(struct work_struct *work)
{
	...
	chg_info("type=[%s,%s], vote_result:%d\n", oplus_wired_get_chg_type_str(chip->wired_type),
			oplus_wired_get_chg_type_str(wired_type), !get_effective_result(chip->pps_boot_votable));
	if (chip->wired_type != wired_type) {
		/* get pps type first time and support pps and pps ready, request pps */
		if (wired_type == OPLUS_CHG_USB_TYPE_PD_PPS) {
			if (!get_effective_result(chip->pps_boot_votable)) {
...
static int oplus_pps_parse_charge_strategy(struct oplus_pps *chip)
{
	...
	chip->pps_third_limit_en = of_property_read_bool(node, "oplus,pps_third_limit_en");
	chg_info("oplus,pps_third_limit_en is %d", chip->pps_third_limit_en);
