...
static struct frame_group *frame_boost_groups[MAX_NUM_FBG_ID];
...
#else
	else if (ots->fbg_state & DEFAULT_FRAME_GROUP_ID)
		return frame_boost_groups[DEFAULT_FRAME_GROUP_ID];
#endif
...
static void add_task_to_frame_group(struct frame_group *grp, struct task_struct *task)
{
	...
	else if (grp == frame_boost_groups[DEFAULT_FRAME_GROUP_ID])
		ots->fbg_state |= FRAME_DEFAULT;
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST_MULTI)
...
void update_ed_task_boost_mid_duration(unsigned int ed_task_boost_mid_duration_scale)
{
	...
	grp = frame_boost_groups[DEFAULT_FRAME_GROUP_ID];
...
static s64 update_window_start(u64 wallclock, struct frame_group *grp)
{
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST_MULTI)
		static unsigned long multi_count[MULTI_FBG_NUM];
#endif
		unsigned long *count;
		char *msg;
		...
		if (grp_id == SF_FRAME_GROUP_ID) {
			count = &sf_count;
			msg = "03_window_start_sf";
		} else if (grp_id == DEFAULT_FRAME_GROUP_ID) {
			count = &main_count;
			msg = "03_window_start_main";
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST_MULTI)
		} else if (is_multi_frame_fbg(grp_id)) {
			count = &multi_count[grp_id - MULTI_FBG_NUM];
			msg = "03_window_start_multi";
#endif
		} else {
			return delta;
		}
		...
		*count += 1;
		val_systrace_c(grp_id, (*count)%2, msg);
	}
...
static void update_group_exectime(struct frame_group *grp, int group_id)
{
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
		val_systrace_c(grp->id, grp->prev_window_exec, "prev_window_exec");
		val_systrace_c(grp->id, get_frame_putil(group_id, grp->prev_window_scale, grp->frame_zone),
			"prev_window_util");
		val_systrace_c(grp->id, grp->curr_window_exec, "curr_window_exec");
...
int rollover_frame_group_window(int grp_id)
{
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
		val_systrace_c(grp_id, get_frame_rate(grp_id), "framerate");
		val_systrace_c(grp_id, grp->ui_pid, "00_ui");
	}
...
static void update_frame_zone(struct frame_group *grp, u64 wallclock)
{
	...
	if (!grp->frame_zone)
		set_frame_state(grp->id, FRAME_END);

	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
		if (grp == frame_boost_groups[SF_FRAME_GROUP_ID])
			sf_zone_systrace_c(grp->id, grp->frame_zone);
		else
			def_zone_systrace_c(grp->id, grp->frame_zone);
	}
}
...
static struct oplus_sched_cluster *best_cluster(struct frame_group *grp)
{
	...
	if (is_multi_frame_fbg(grp->id)) {
		if (util >  stune_boost[BOOST_UTIL_MIN_THRESHOLD])
			boosted_margin_util = schedtune_grp_margin(util, stune_boost[BOOST_DEF_MIGR]);
		boosted_grp_util += boosted_margin_util;

		if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE))
			val_systrace_c(grp->id, boosted_margin_util ? stune_boost[BOOST_DEF_MIGR] : 0, "cfs_boost_migr");
	}
#else
	if (grp->id == DEFAULT_FRAME_GROUP_ID)
	...
	unsigned int flags)
{
	...
	vutil = get_frame_vutil(grp->id, timeline);
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE) && (grp != frame_boost_groups[SF_FRAME_GROUP_ID])) {
			val_systrace_c(grp->id, vutil, "vutil");
			val_systrace_c(grp->id, use_vutil, "use_vutil");
	}
	...
	unsigned long *util)
{
	...
	for (i = MULTI_FBG_ID; i <  MULTI_FBG_ID + MULTI_FBG_NUM; i++) {
		...
		if (list_empty(&grp->tasks))
			goto unlock_fbg;
		if (!grp->preferred_cluster || (get_frame_state(i) == FRAME_END))
			goto unlock_fbg;
		 ...
		 */
			...
			if (policy_util >  stune_boost[BOOST_UTIL_MIN_THRESHOLD])
				boosted_margin_util = schedtune_grp_margin(policy_util, stune_boost[BOOST_DEF_FREQ]);
...
unlock_fbg:
		...
		*util = max(*util, boosted_policy_util);
		...
		if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
			...
			cpu_util_systrace_c(grp->id, boosted_policy_util * 10000 + curr_util, first_cpu, "cfs_policy/curr_util");
			val_systrace_c(grp->id, boosted_margin_util ? stune_boost[BOOST_DEF_FREQ] : 0, "cfs_boost_freq");
...
unlock_fbg:
	...
	*util = max(raw_util, boosted_policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
		unsigned long curr_util = policy_util ? atomic64_read(&grp->curr_util) : 0;
		cpu_util_systrace_c(grp->id, boosted_policy_util * 10000 + curr_util, first_cpu, "cfs_policy/curr_util");
		val_systrace_c(grp->id, stune_boost[BOOST_DEF_FREQ], "cfs_boost_freq");
		if (boosted_policy_util == *util)
			max_grp_id = grp->id;
	}
#endif
	...
	*util = max(*util, boosted_policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
		int boost_sf;
		unsigned long curr_util = policy_util ? atomic64_read(&grp->curr_util) : 0;
		cpu_util_systrace_c(grp->id, boosted_policy_util * 10000 + curr_util, first_cpu, "rt_policy/curr_util");
		...
		val_systrace_c(grp->id, boost_sf, "rt_boost_freq/migr");
		if (boosted_policy_util == *util)
			max_grp_id = grp->id;
	}
	...
	*util = max(*util, policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
		cpu_util_systrace_c(grp->id, policy_util, first_cpu, "IMS_policy_util");
		if (policy_util == *util)
			max_grp_id = grp->id;
...
void inputmethod_update_cpufreq(int grp_id, struct task_struct *tsk)
{
	...
	if (grp_id == INPUTMETHOD_FRAME_GROUP_ID) {
		struct frame_group *grp = frame_boost_groups[INPUTMETHOD_FRAME_GROUP_ID];
		unsigned long flags;
		u64 wallclock = fbg_ktime_get_ns();
...
void inputmethod_set_boost_start(int grp_id)
{
	...
	if (ui) {
		inputmethod_update_cpufreq(grp_id, ui);
		put_task_struct(ui);
	}
}
	...
	u64 wallclock, struct frame_group *grp)
{
	...
	if (likely(delta_wc_ws >= running)) {
		...
		if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE)) {
			val_systrace_c(grp->id, grp->prev_window_exec, "prev_window_exec");
			val_systrace_c(grp->id, get_frame_putil(grp->id, grp->prev_window_scale, grp->frame_zone), "prev_window_util");
			val_systrace_c(grp->id, grp->curr_window_exec, "curr_window_exec");
			val_systrace_c(grp->id, get_frame_putil(grp->id, grp->curr_window_scale, grp->frame_zone), "curr_window_util");
		}
	}
...
skip:
	if (need_freq_update) {
		if (grp->id == SF_FRAME_GROUP_ID)
			...
			inputmethod_update_cpufreq(grp->id, tsk);
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST_MULTI)
		else if (is_multi_frame_fbg(grp->id))
#else
		else if (grp->id == DEFAULT_FRAME_GROUP_ID)
#endif
...
bool set_frame_group_task_to_perfer_cpu(struct task_struct *p, int *target_cpu)
{
	...
	if ((grp->id != DEFAULT_FRAME_GROUP_ID) &&
			(grp->id != INPUTMETHOD_FRAME_GROUP_ID) &&
