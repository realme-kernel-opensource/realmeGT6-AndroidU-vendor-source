...
//fangzhijun@CONNECTIVITY.WIFI.HARDWARE.SWITCH.5719240, 2023/07/11,
...
static int hdd_initialize_mac_address(struct hdd_context *hdd_ctx)
{
	...
	if ((!qdf_is_macaddr_zero(&hdd_ctx->hw_macaddr)) && (!qdf_is_macaddr_Fw_Default(&hdd_ctx->hw_macaddr))) {
		...
		//fangzhijun@CONNECTIVITY.WIFI.HARDWARE.MAC.5719240, 2023/06/02, check if mac address is legal or not. If not, revert it.
