...
static int cnss_pci_runtime_resume(struct device *dev)
{
	...
	//fangzhijun@CONNECTIVITY.WIFI.HARDWARE.Log.5719240 2023/06/20,not print so much logs
...
static int cnss_pci_enumerate(struct cnss_plat_data *plat_priv, u32 rc_num)
{
	...
	#ifdef OPLUS_FEATURE_WIFI_DCS_SWITCH
	//TAO.HUI@CONNECTIVITY.WIFI.HARDWARE.SWITCH.2932985, 2022/11/27,
	//Add for wifi switch monitor
...
retry:
	...
	if (ret) {
		...
		if (retry++ < LINK_TRAINING_RETRY_MAX_TIMES) {
			...
			#ifdef OPLUS_FEATURE_WIFI_DCS_SWITCH
			//TAO.HUI@CONNECTIVITY.WIFI.HARDWARE.SWITCH.2932985, 2022/6/27,
			//Add for wifi switch monitor
