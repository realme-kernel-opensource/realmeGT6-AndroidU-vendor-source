...
#include "dsi_iris_memc_helper_i8.h"
...
static void _iris_dsc_up_format(bool up, uint32_t *payload, uint32_t pps_sel)
{
	...
	case CHIP_IRIS7P:
		break;
	case CHIP_IRIS8:
		iris_dsc_up_format_i8(up, payload, pps_sel);
		break;
	default:
...
static void _iris_dsc_update_ctrl(uint32_t pps_sel)
{
	...
	_iris_dsc_up_format(iris_dsc_info[pps_sel].chroma_format == MSM_CHROMA_422,
					payload, pps_sel);
		...
		uint8_t ip_src, uint8_t opt_src)
{
	...
	for (i = 0; i < src_popt->cmd_cnt; i++) {
		...
		IRIS_LOGD("%s(), offset: %u, cmd buf len: %u", __func__,
...
static bool _iris_scl_lp_memc(void)
{
	...
	changed |= iris_ioinc_pp_proc_i8(true, ioinc_pos,
			pcfg->frc_setting.mv_hres, pcfg->frc_setting.mv_vres,
			pcfg->frc_setting.disp_hres, pcfg->frc_setting.disp_vres,
		...
		uint32_t scl_type)
{
	...
	changed = iris_ioinc_pp_proc_i8(true, values[3], values[1], values[2],
			pcfg->frc_setting.disp_hres, pcfg->frc_setting.disp_vres,
		...
		uint32_t scl_type)
{
	...
	changed |= iris_ioinc_pp_proc_i8(true, values[6], values[3], values[4],
			pcfg->frc_setting.disp_hres, pcfg->frc_setting.disp_vres,
			path_sel, sel_mode, &blend_changed);
...
void iris_scl_ioinc_pp_filter(uint32_t count, uint32_t *values)
{
	...
	iris_ioinc_pp_filter_i8(count, values);
}
...
void iris_set_pp_scl_aux(void)
{
	...
	if (pcfg->panel2 == NULL || pcfg->panel2->cur_mode == NULL)
		return;

	if (pcfg->panel2->cur_mode->timing.h_active == 0 ||
			pcfg->panel2->cur_mode->timing.v_active == 0)
		return;
	...
	IRIS_LOGI("%s(), enable pp scl %u for secondary display %ux%u",
			__func__, ioinc_pos,
			pcfg->panel2->cur_mode->timing.h_active,
			pcfg->panel2->cur_mode->timing.v_active);

	if (ioinc_pos == SCL_PP_BLD_OUT) {
		iris_aux_h = pcfg->panel2->cur_mode->timing.h_active;
		iris_aux_v = pcfg->panel2->cur_mode->timing.v_active;
	}
...
int iris_dbgfs_scl_init(struct dsi_display *display)
{
	...
	if (iris_chip_type == CHIP_IRIS8)
		if (debugfs_create_file("blend_scl_pos", 0644, pcfg->dbg_root, display,
