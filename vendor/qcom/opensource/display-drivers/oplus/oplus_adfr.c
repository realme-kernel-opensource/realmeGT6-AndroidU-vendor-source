...
int oplus_adfr_get_test_te(void *buf)
{
	...
	struct dsi_display *display = oplus_display_get_current_display();
	struct oplus_adfr_params *p_oplus_adfr_params = NULL;
	static unsigned int last_report_refresh_rate = 0;
	...
	/* Reduce log printing and print only when the reported value changes. */
	if (last_report_refresh_rate != *refresh_rate) {
		ADFR_INFO("The reported value has changed, oplus_adfr_test_te_refresh_rate:%u\n", *refresh_rate);
	}

	last_report_refresh_rate = *refresh_rate;
